...
fn parse_simple_version(line: &str) -> VersionSpec {
    ...
    // Advance start until the next non-blank char
    loop {
        let chr = &line.chars().nth(start);
        if chr.is_none() {
            break;
        }
        let chr = chr.unwrap();
    ...
    let mut end = start;
    loop {
        let chr = &line.chars().nth(end);
        if chr.is_none() {
            break;
        }
        let chr = chr.unwrap();
        if chr.is_ascii_whitespace() || (chr == '\\') || (chr == '#') {
            break;
        }
        end += 1;
    }
...
fn parse_git_ref(line: &str) -> Result<VersionSpec, Error> {
    ...
    if chunks.len() != 2 {
        return Err(Error::MalformedLock {
            details: format!("expecting `<ref>#egg=<name>` after `@`, got '{}'", after_at),
        });
    }
